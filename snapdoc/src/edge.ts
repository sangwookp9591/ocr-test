...
/** 최대값 대비 비율 임계로 이진화 */
export function threshold(g: Gray, ratio = 0.25): Uint8Array {
  let max = 0;
  for (let i = 0; i < g.data.length; i++) if (g.data[i] > max) max = g.data[i];
  const t = max * ratio;
  const out = new Uint8Array(g.data.length);
  for (let i = 0; i < g.data.length; i++) out[i] = g.data[i] > t ? 1 : 0;
  return out;
}
