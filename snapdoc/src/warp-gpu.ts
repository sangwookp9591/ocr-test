...
let devicePromise: Promise<any | null> | null = null;

function getDevice(): Promise<any | null> {
  if (!devicePromise) {
    const gpu = (navigator as any).gpu;
    devicePromise = gpu
      ? gpu.requestAdapter().then((a: any) => (a ? a.requestDevice() : null)).catch(() => null)
...
export async function warpGpu(
  ...
  H: Float64Array, dstW: number, dstH: number,
