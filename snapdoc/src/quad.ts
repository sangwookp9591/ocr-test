 ...
 */
export function findQuad(edges: Uint8Array, w: number, h: number): Quad | null {
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      if (a > sTl) { sTl = a; tl = { x, y }; }
      if (b > sTr) { sTr = b; tr = { x, y }; }
      if (c > sBr) { sBr = c; br = { x, y }; }
      if (d > sBl) { sBl = d; bl = { x, y }; }
    }
  }
  ...
  if (area < w * h * 0.1 || area > w * h * 0.95) return null;

  // 코너가 프레임 코너와 사실상 일치(평균 이탈 <5% 대각선)면 크롭 무의미 → null
  const diag = Math.hypot(w, h);
  const drift =
    (Math.hypot(tl.x, tl.y) + Math.hypot(w - tr.x, tr.y) +
     Math.hypot(w - br.x, h - br.y) + Math.hypot(bl.x, h - bl.y)) / 4;
  if (drift < diag * 0.05) return null;
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (Math.abs(cos) > 0.57) return null; // |cos| 0.57 ≈ 55°/125° 한계
  }
  return quad;
}
...
function cornerCos(p: Point, q: Point, r: Point): number {
