 ...
 */
export function computeH(src: Quad, dstW: number, dstH: number): Float64Array {
  const d: Point[] = [
    { x: 0, y: 0 }, { x: dstW - 1, y: 0 },
    { x: dstW - 1, y: dstH - 1 }, { x: 0, y: dstH - 1 },
  ];
  const s: Point[] = [src.tl, src.tr, src.br, src.bl];
...
/** H 적용: dst 좌표 → src 좌표 */
export function applyH(H: Float64Array, x: number, y: number): Point {
