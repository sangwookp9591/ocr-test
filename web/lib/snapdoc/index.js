...
function toCanvas(src, maxSize) {
    ...
    if (scale === 1 && src instanceof HTMLCanvasElement)
        return src; // 이미 적정 캔버스 — 재복사 생략
    const c = document.createElement('canvas');
...
export function quality(src, opts = {}) {
...
export function highlight(video, overlay) {
    ...
    const loop = (t) => {
        ...
        if (t - last < 66)
            return; // 감지는 ~15fps면 충분 — 60fps 풀 파이프라인은 낭비
        last = t;
        if (!video.videoWidth)
            return;
        if (overlay.width !== video.videoWidth)
            overlay.width = video.videoWidth; // 대입만으로도
        if (overlay.height !== video.videoHeight)
            overlay.height = video.videoHeight; // 백킹스토어 재할당됨
